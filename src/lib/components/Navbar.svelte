<script lang="ts">
  ...
  const navLinkClass = cn(
    ...
    'hover:bg-transparent hover:decoration-(--accent-color) focus:bg-transparent',
    'data-[active]:decoration-(--accent-color) data-[active]:decoration-2'
  );
</script>

<NavigationMenu.Root class={cn('relative z-10 max-w-full justify-start pt-4 border-b border-(--accent-color) bg-background')}>
  ...
  <NavigationMenu.List class={cn('max-lg:absolute max-lg:left-1/2 max-lg:top max-lg:-translate-x-1/2 max-lg:-translate-y-1/2')}>
    <NavigationMenu.Item>
      <NavigationMenu.Link href="/" onclick={selectSection('#home')} class={cn('justify-center rounded-b-none font-extrabold', 'w-50 bg-(--accent-color)!')}>
        betosardinha.dev
      </NavigationMenu.Link>
    </NavigationMenu.Item>
  </NavigationMenu.List>
  ...
  <NavigationMenu.List
    class={cn(
      'max-lg:absolute max-lg:left-0 max-lg:top-full max-lg:w-full max-lg:flex-col max-lg:items-center max-lg:gap-4 max-lg:border-t max-lg:border-b max-lg:border-(--accent-color) max-lg:bg-background max-lg:py-4 lg:flex',
